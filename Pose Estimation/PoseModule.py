...
class poseDetector():
    ...
    def __init__(self, mode=False, model_complexity=1, smooth=True, enableSeg=False,
        ...
        self.pose = self.mpPose.Pose(self.mode, self.model_complexity, self.smooth, self.enableSeg, self.smoothSeg, self.detectionCon, self.trackCon)

    def findPose(self, img, draw=True):
        imgRGB = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
        # Get detection of pose
        self.results = self.pose.process(imgRGB)

        # Draw lamdmarks (Add mpPose.POSE_CONNECTIONS for connection lines)
